function searchHits(searchStr, canRecurse) {
	var maxNumResults = 25,			// maximum number of results returned, pre-filter options
		tfidfThreshold = 0.45,		// threshold value to filter out bad matches by
		relevantTFIDFFactor = 0.69,	// adjustment for recursive search; properties are weighted lower
		dontRecurse = false,		// used for child recursion calls
		i,
		j,
		...
		bestElement,				// highest rated relevant element
		currentElement,
		relevantSearchArray = [],
		tempResultsArray = [],
		relevantResultsArray = [],	// ImageElement index values that are related to the query
		shouldKeep;
	...
	for(i = 0; i < filteredSearchArray.length; i++) {
		...
		if (matchArray) {
			...
			for(j = 0; j < freqArrays[0].length; j++) {
				...
				// recursed searches should have lower values than original search
				if(!canRecurse)
					tfidf = (tfidf * relevantTFIDFFactor).toFixed(2);
	...
	for(i = 0; i < maxNumResults && i < tfidfSortedArray.length; i++) {
		console.log(tfidfSortedArray[i]);
		//debugger;
	...
	// check if the results met the limit, if they didnt,
	// run a recursive search on similar terms,
	// adding new ones to the end of the search in order
	if (resultsArray.length < maxNumResults && resultsArray.length > 0 && canRecurse) {
		//debugger;
		bestElement = imageArray[resultsArray[0]];

		// search by categories and location
		relevantSearchArray = bestElement.categories.slice();
		relevantSearchArray.push(bestElement.location);

		for(i = 0; i < relevantSearchArray.length; i++){
			tempResultsArray = searchHits(relevantSearchArray[i], dontRecurse);

			// add results to relevantResultsArray
			for(j = 0; j < tempResultsArray.length; j++) {
				relevantResultsArray.push(tempResultsArray[j]);
			}
		}

		// grab each element one at a time from relevant results,
		// check for dupe in results and add to end of results array if no dupes
		for(i = 0; i < relevantResultsArray.length; i++){
			shouldKeep = true;
			currentElement = relevantResultsArray[i];
			for(j = 0; j < resultsArray.length; j++) {
				if(resultsArray[j] === currentElement){
					shouldKeep = false;
					break;
				}
			}

			// if no duplicate then add it to array
			if(shouldKeep) {
...
var invIdx = {
	...
	"corona":[2],
	"costa":[0,9],
	"cupcake":[0, 10,9,9,10,11, 11, 12, 12, 12],
	"dees":[11],
	"dessert":[0,9],
	"farmer":[5],
	"fashion":[9],
